...
const AddForm = () => {
  ...
  return (
    <div className="flex items-center justify-center py-10 min-h-screen flex-col bg-[#d9d9d9]">
      <div className="bg-white shadow-lg rounded-[0.5rem] p-8 max-w-lg w-full text-center transition-transform duration-300 transform hover:scale-105 border border-gray-300 -mt-20">
        <h1 className="text-3xl font-bold text-gray-800 mb-6">Tambah {type === 'barang' ? 'Barang' : 'Ruangan'}</h1>

        <div className="flex justify-center space-x-4 mb-6">
          <button
            onClick={() => setType('barang')}
            className={`px-6 py-2 rounded-[0.5rem] text-gray-700 font-semibold ${
              type === 'barang' ? 'bg-red-600 text-white shadow-md' : 'bg-gray-200 hover:bg-gray-300'
          ...
          <button
            ...
            className={`px-6 py-2 rounded-[0.5rem] text-gray-700 font-semibold ${
              type === 'ruangan' ? 'bg-red-600 text-white shadow-md' : 'bg-gray-200 hover:bg-gray-300'
            } transition duration-200`}
          >
            Ruangan
          </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {type === 'barang' ? (
            <>
              <input
                ...
                className="w-full px-4 py-2 border rounded-[0.5rem] shadow-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-red-400"
              />
              <input
                type="number"
                value={jumlah}
          ...
          <button
            type="submit"
            className={`w-full py-3 text-white rounded-[0.5rem] ${
